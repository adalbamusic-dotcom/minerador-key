/**
 * ===== A ÚNICA PORTA DA TELA PARA A FRONTEIRA — RADAR_FINAL_1.2 · §2 =====
 *
 * ================== POR QUE ISTO EXISTE COMO MÓDULO ==================
 *
 * Três lugares mandavam artigos do Radar para o Planejador: o botão da
 * investigação, o botão da análise e a ação em lote da planilha. Os dois
 * últimos passavam por `importApprovedToPlanner`, que move a esteira e nada
 * mais — sem dossiê, sem prontidão, sem identidade de ArticleDNA.
 *
 * Um artigo que sai por ali chega ao Planejador SEM evidência atrás, e nada na
 * tela diria isso: o item aparece lá como qualquer outro.
 *
 ...
 */
export async function postRadarPlannerHandoff(input: {
  brandId: string;
  articleId: string;
  ...
  try {
    const resposta = await chamar("/api/editorial/radar-planner-handoff", {
      method: "POST",
    ...
    if (!resposta.ok || !corpo?.success) {
      ...
      return {
        ...
        message: typeof corpo?.error === "string" ? corpo.error : "Não foi possível enviar ao Planejador.",
        code,
      };
    }
    ...
    return {
      ...
      message: typeof corpo.headline === "string" ? corpo.headline : "Pacote enviado ao Planejador.",
      code: null,
    };
  } catch (erro) {
    return {
      articleId: input.articleId,
      outcome: "FAILED",
      message: erro instanceof Error ? erro.message : "Falha no envio ao Planejador.",
      code: null,
    };
  }
}
 ...
 *
 * Ele não monta dossiê, não grava dossiê, não decide prontidão e não toca no
 * Planejador. Ele repete a mesma porta e junta os desfechos.
 *
 ...
 */
export async function postRadarPlannerHandoffBatch(input: {
  brandId: string;
  ...
  for (const articleId of input.articleIds) {
    resultados.push(await postRadarPlannerHandoff({
 ...
 * diferente.
 */
export function radarHandoffBatchSummary(resultados: readonly RadarHandoffItemResult[]): string {
