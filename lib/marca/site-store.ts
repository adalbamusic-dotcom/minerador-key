...
export function siteWorkspaceStorageKey(actorUserId: string, brandId: string): string {
  return `minerador-pro:site-workspace:${actorUserId}:${brandId}`;
}
...
export async function loadBrandSiteWorkspace(actorUserId: string, brandId: string): Promise<{ workspace: BrandSiteWorkspace; parseError: string | null }> {
  const raw = await readBrowserArtifact(siteWorkspaceStorageKey(actorUserId, brandId));
...
export async function saveBrandSiteWorkspace(actorUserId: string, workspace: BrandSiteWorkspace): Promise<"indexeddb" | "localstorage"> {
  const validated = BrandSiteWorkspaceSchema.parse({ ...workspace, persistenceMode: "local_fallback", updatedAt: new Date().toISOString() });
  return writeBrowserArtifact(siteWorkspaceStorageKey(actorUserId, validated.brandId), validated);
}
