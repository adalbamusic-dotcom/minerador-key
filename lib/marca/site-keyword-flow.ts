...
export type SiteKeywordImportCandidate = Pick<SiteKeywordCandidate, "id" | "text" | "normalizedText" | "catalogEntryId" | "sourceUrl" | "sourceField" | "suggestedRole" | "confidence"> & Partial<Pick<SiteKeywordCandidate, "sourceFields" | "slugCoherence" | "urlSituation" | "publicationStatus" | "keywordUrlRelation" | "architectureStatus" | "relationConfirmedBy" | "relationConfirmedAt" | "extractedAt">> & { resolvedUrl?: string | null; declaredCanonicalUrl?: string | null };
...
export function buildSiteKeywordImportPlan(candidates: SiteKeywordImportCandidate[], existingRows: SiteKeywordImportExisting[]): { items: SiteKeywordImportPlanItem[]; summary: SiteKeywordImportSummary } {
  ...
  const items = candidates.map(candidate => {
    ...
    const base = { candidateId: candidate.id, text: candidate.text, normalizedText, catalogEntryId: candidate.catalogEntryId, sourceUrl: candidate.sourceUrl, sourceField: candidate.sourceField, sourceFields: candidate.sourceFields?.length ? candidate.sourceFields : [candidate.sourceField], suggestedRole: candidate.suggestedRole, slugCoherence: candidate.slugCoherence || "unknown" as const, urlSituation: candidate.urlSituation || "unverified" as const, publicationStatus: candidate.publicationStatus || "not_confirmed" as const, keywordUrlRelation: candidate.keywordUrlRelation || "undefined" as const, architectureStatus: candidate.architectureStatus || "awaiting_architecture" as const, relationConfirmedBy: candidate.relationConfirmedBy ?? null, relationConfirmedAt: candidate.relationConfirmedAt ?? null, resolvedUrl: candidate.resolvedUrl ?? null, declaredCanonicalUrl: candidate.declaredCanonicalUrl ?? null, extractedAt: candidate.extractedAt ?? null, confidence: candidate.confidence };
