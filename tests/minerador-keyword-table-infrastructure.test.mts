...
test("Descoberta usa colunas contratadas sem workflow do Processador", () => { for (const column of ["Keyword", "Relação", "Resultados", "Volume", "CPC", "Concorrência Ads", "Intenção preliminar", "Funil preliminar", "Situação"]) assert.match(discovery, new RegExp(column)); for (const removed of ["Histórico", "Targeting"]) assert.doesNotMatch(discovery, new RegExp(`label: "${removed}"`)); assert.match(discovery, /KeywordTableBulkBarShell/); assert.match(discovery, /useKeywordTableSelection/); assert.doesNotMatch(discovery, /Principal|KGR|Silo\/Categoria|Status editorial|supabase|DiscoveryRun|createGoogleAds/); assert.match(discovery, /dataforseo\/allintitle/); assert.match(discovery, /Medir resultados/); assert.match(discovery, /\/discovery\/import/); });
test("Descoberta prioriza Keyword sem alterar o grid do Processador", () => {
  assert.match(discovery, /discoveryColumnWidths/);
  // 2026-09-24, pedido do dono: Histórico e Targeting saíram e a Keyword nunca é cortada:
  // sem teto de 380px nem truncate; ela recebe o espaço que sobra.
  assert.match(discovery, /return columnId === "keyword" \? undefined : 1;/);
  assert.doesNotMatch(discovery, /const keywordCell = "[^"]*(?:truncate|max-w-)/);
