...
export default async function Page({ params }: { params: Promise<{ brandRef: string; articleId: string }> }) {
  ...
  return <Suspense fallback={<div className="p-6 text-sm text-text-muted">Carregando análise do Radar…</div>}><RadarAnalysisPage brandRef={brandRef} articleId={decodeURIComponent(articleId)} /></Suspense>;
}
