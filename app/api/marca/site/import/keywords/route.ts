  ...
  try {
    ...
    const result = await importSiteKeywordsToMinerador({
      ...
      repository: {
        ...
        findByIds: async (requestedBrandId, ids) => {
          if (requestedBrandId !== brandId) throw new Error("A marca da consulta nao corresponde ao tenant autorizado.");
          const { data, error } = await brand.profile.supabase.from("minerador_keywords").select("id,brand_id,keyword,status,analise_semantica").eq("brand_id", brandId).is("deleted_at", null).in("id", [...ids]);
