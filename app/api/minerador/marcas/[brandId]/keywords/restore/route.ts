...
export async function POST(request: Request, { params }: { params: Promise<{ brandId: string }> }) {
  try {
    ...
    const { data, error } = await profile.supabase.rpc("lifecycle_restore_minerador_keywords", {
      p_brand_id: brandId,
    ...
    if (error instanceof z.ZodError) return NextResponse.json({ success: false, code: "KEYWORD_RESTORE_FAILED", message: "Seleção de keywords inválida." }, { status: 400 });
    return lifecycleErrorResponse(error, "KEYWORD_RESTORE_FAILED");
  }
}
