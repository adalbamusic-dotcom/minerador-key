...
-- Successor of the already-applied InternalLinkGraph foundation.
-- The advisory transaction lock below serializes the same Brand/graph
-- lineage, so the row-level FOR UPDATE is redundant and would require
-- UPDATE privilege on the append-only graph table.
