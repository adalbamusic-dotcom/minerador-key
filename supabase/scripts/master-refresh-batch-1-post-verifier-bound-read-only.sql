-- MASTER REFRESH / BATCH 1 — post-verifier remoto bound, somente leitura.
-- Projeto esperado: hjjlntdpdgvpnazdztqw
-- Baseline real capturada em 2026-08-17 depois da aprovação dos quatro gates.
-- Executar após o deploy do Batch 1. O banco deve permanecer idêntico à baseline PRE.
...
SELECT
  CASE WHEN cardinality(mismatches) = 0 THEN 'PASS' ELSE 'FAIL' END AS post_verifier_status,
  mismatches,
