-- Local rollback artifact for 0032.
-- DO NOT EXECUTE automatically. Requires a new snapshot, incident decision
-- and human authorization. It recreates empty historical structures only;
-- it does not restore grants, execution events or rollback rows.

BEGIN;

DO $$
BEGIN
  IF pg_catalog.to_regprocedure('public.pipeline_editorial_protect_append_only()') IS NULL THEN
    RAISE EXCEPTION '0032_ROLLBACK_BLOCKED: shared append-only function is missing';
  END IF;
END
$$;
...
CREATE TRIGGER brand_exceptional_operation_execution_events_append_only_trg_00
...
AS $$
  SELECT target_brand_id IS NOT NULL
    ...
    AND EXISTS (
      SELECT 1
      FROM public.brand_exceptional_operation_grants AS grant_record
...
CREATE TABLE public.tenant_0016_agency_role_rollback (
