...
export function ProfessionalWriter({ initialArticleId = null, initialDocumentId = null }: { initialArticleId?: string | null; initialDocumentId?: string | null }) {
  ...
  return <div className={`${fullscreen ? "fixed inset-x-0 bottom-0 top-10 z-30" : "h-screen"} flex min-h-0 flex-col bg-[#07080b]`}>
    ...
    {selected && <footer className="flex shrink-0 items-center justify-between gap-3 overflow-x-auto border-t border-context-accent/30 bg-[#0b0c10] px-3 py-2"><div className="flex shrink-0 items-center gap-2"><WorkflowStatusBadge status={selected.status}/><span className="text-[9px] text-slate-500">Perfis das keywords: {selected.keywordDnaRefs.length} · Definição do artigo {selected.articleDnaRef.versionId} · Arquitetura do silo {selected.siloDnaRef.versionId}</span></div><div className="flex shrink-0 items-center gap-2"><button className={tool} disabled={selected.status === "aprovado"} onClick={() => requestStatus("em_revisao")}>Enviar para aprovação</button><button className={tool} disabled={selected.status !== "em_revisao"} onClick={() => requestStatus("aprovado")}>Aprovar para Publicações</button></div></footer>}
    <WorkflowImportDialog open={importOpen} title="Importar artigos aprovados do Planejador" description="Todos os planos editoriais aprovados aparecem aqui. Os já importados continuam visíveis e identificados." rows={availablePlans} label={item => item.title} details={item => <span className="mt-1 block text-slate-500">/{item.slug} · {item.intent}</span>} disabled={item => item.alreadyImported} status={item => item.alreadyImported ? "sent_writer" : "approved"} onClose={() => setImportOpen(false)} onImport={importFromPlanner}/>
  </div>;
}
