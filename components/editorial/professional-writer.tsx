...
import { AlignCenter, AlignLeft, AlignRight, Bold, ChevronLeft, ChevronRight, Columns3, Eraser, ExternalLink, FileText, Fullscreen, Heading1, Heading2, Heading3, Heading4, Image, Italic, Link2, List, ListOrdered, MessageSquare, Minus, Pilcrow, Plus, Quote, Redo2, Search, Sparkles, Strikethrough, Table2, UnderlineIcon, Undo2, WandSparkles } from "lucide-react";
...
export function ProfessionalWriter({ initialArticleId = null, initialDocumentId = null }: { initialArticleId?: string | null; initialDocumentId?: string | null }) {
  const pipeline = useEditorialPipeline(); const { selectedBrandId, activeBrand } = useBrand();
  ...
  useEffect(() => { if (!selected || !editor) return; const timer = window.setTimeout(() => { const raw = window.localStorage.getItem(`minerador-pro:document-recovery:${selectedBrandId}:${selected.id}`); if (!raw) return;
    ...
    }, 0); return () => window.clearTimeout(timer); }, [editor, selected?.id, selectedBrandId]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  useEffect(() => { if (!pending) return; const timer = window.setTimeout(async () => { setSaveState("saving"); setSaveMessage("Salvando…"); const hash = await contentHash(pending);
      try { const response = await fetch("/api/editorial/documents", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ brandId: selectedBrandId, documentId: pending.id, expectedLockVersion: pipeline.documentLocks[pending.id] || 1, document: pending, contentHash: hash, createVersion: createVersionRef.current, changeReason: createVersionRef.current ? "Marco editorial criado pelo usuário." : "Autosave editorial." }) }); const body = await response.json();
        if (response.ok) { pipeline.updateDocumentLocal(pending, body.lockVersion); window.localStorage.removeItem(`minerador-pro:document-recovery:${selectedBrandId}:${pending.id}`); setSaveState("saved_server"); setSaveMessage(body.version ? `Versão ${body.version.versionNumber} criada e salva.` : `Salvo no servidor às ${new Date(body.updatedAt).toLocaleTimeString("pt-BR")}.`); createVersionRef.current = false; editCheckpointRef.current = false; setPending(null); return; }
        ...
        window.localStorage.setItem(`minerador-pro:document-recovery:${selectedBrandId}:${pending.id}`, JSON.stringify(pending)); setSaveState("saved_local"); setSaveMessage(body.code === "persistence_unavailable" ? "Salvo somente como recuperação local; migration ainda não aplicada." : "Servidor indisponível; recuperação local criada.");
      } catch { window.localStorage.setItem(`minerador-pro:document-recovery:${selectedBrandId}:${pending.id}`, JSON.stringify(pending)); setSaveState("saved_local"); setSaveMessage("Sem conexão; recuperação local criada."); }
    }, 1200); return () => window.clearTimeout(timer); }, [pending, selectedBrandId, pipeline]);
  ...
  return <div className={`${fullscreen ? "fixed inset-0 z-[100]" : "h-screen"} flex min-h-0 flex-col bg-[#07080b]`}>
    <header className="flex h-11 shrink-0 items-center gap-2 overflow-x-auto border-b border-slate-850 px-2"><FileText className="h-4 w-4 shrink-0 text-indigo-400"/><div className="min-w-0"><p className="truncate text-[10px] font-bold text-white">{selected?.title || "Redator"}</p><p className="text-[8px] text-slate-600">{activeBrand?.nome || "Sem marca"} · {pipeline.persistenceMode === "server" ? "persistência ativa" : "fallback local"}</p></div>{selected && <WorkflowStatusBadge status={selected.status}/>}<span className={`ml-auto shrink-0 text-[9px] ${saveState === "conflict" || saveState === "error" ? "text-red-400" : saveState === "saved_server" ? "text-emerald-400" : "text-amber-400"}`}>{saveMessage || "Pronto"}</span><span className="shrink-0 text-[9px] text-slate-600">{wordCount} palavras</span><HistoryControls entries={history.entries} canUndo={history.canUndo} canRedo={history.canRedo} onUndo={history.undo} onRedo={history.redo} onRestore={history.restore} compact/><button className={tool} onClick={() => setImportOpen(true)}><Plus className="mr-1 h-3 w-3"/>Importar do Planejador</button><button className={tool} onClick={() => setFullscreen(value => !value)} title="Tela cheia"><Fullscreen className="h-3 w-3"/></button><Link href="/publicacoes" className={tool}>Publicações</Link></header>
