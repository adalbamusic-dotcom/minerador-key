...
export function BackgroundTaskNotice({ tasks, onDismiss }: {
  ...
  return <aside className="fixed bottom-14 right-4 z-[70] flex w-[min(390px,calc(100vw-2rem))] flex-col gap-2" aria-live="polite" aria-label="Tarefas do Arquiteto">
    {tasks.map(task => {
      ...
      return <div key={task.id} className={`rounded-md border bg-[#0b0c10] px-3 py-2.5 shadow-2xl ${failed ? "border-rose-900/70" : task.status === "completed" ? "border-emerald-900/70" : "border-indigo-900/70"}`}>
        <div className="flex items-start gap-2">
          {running ? <Loader2 className="mt-0.5 h-3.5 w-3.5 shrink-0 animate-spin text-indigo-400"/>
          ...
          <div className="min-w-0 flex-1">
            ...
            {running && <div className="mt-2 h-1 overflow-hidden rounded bg-slate-900"><div className="h-full bg-indigo-500 transition-[width]" style={{ width: `${percent}%` }}/></div>}
          </div>
        </div>
      </div>;
    })}
