...
export function HistoryControls<T>({ entries, canUndo, canRedo, onUndo, onRedo, onRestore, compact = false, density = "compact", presentation = "drawer", moduleId, showHistory = true, showUndoRedo = true, visualVariant = "legacy" }: {
  ...
  return <div ref={triggerRef}>
    <div className="flex items-center gap-1">
      ...
      {showHistory && <button className={control} onClick={() => { popoverTriggerRef.current = triggerRef.current; setOpen(value => !value); }} title="Ver histórico de segurança" aria-expanded={open}><History className="h-3 w-3"/>Histórico ({entries.length})</button>}
    </div>
    {open && presentation === "drawer" && <div className={`fixed inset-0 z-[120] flex justify-end ${visualVariant === "semantic" ? semanticPanel.drawer : "bg-black/60"}`} role="dialog" aria-modal="true" aria-label="Histórico de segurança"><aside className={`flex h-full w-[min(430px,100vw)] flex-col border-l shadow-2xl ${visualVariant === "semantic" ? semanticPanel.drawerAside : "border-slate-800 bg-[#090a0e]"}`}>{panel}</aside></div>}
    <AnchoredPopover open={open && presentation === "popover"} onClose={() => setOpen(false)} triggerRef={popoverTriggerRef} ariaLabel="Histórico de segurança" className={`flex w-[min(430px,calc(100vw-1rem))] flex-col p-0 ${visualVariant === "semantic" ? semanticPanel.popover : "border-slate-800 bg-[#090a0e]"}`}>{panel}</AnchoredPopover>
  </div>;
}
...
function HistoryPanel<T>({ entries, onRestore, onClose, control, visualVariant }: { entries: Array<EditorialHistoryEntry<T>>; onRestore: (id: string) => void; onClose: () => void; control: string; visualVariant: "legacy" | "semantic" }) {
  ...
  return <>
    <header className={`flex items-start justify-between border-b p-4 ${panel.header}`}><div><p className={`text-xs font-bold uppercase tracking-widest ${panel.title}`}>Histórico de segurança</p><h2 className={`mt-1 text-sm font-bold ${panel.heading}`}>Alterações desta sessão</h2><p className={`mt-1 text-sm leading-5 ${panel.description}`}>Restaurar cria antes um ponto de retorno. Nenhuma restauração altera publicados protegidos no banco.</p></div><button className={control} onClick={onClose} aria-label="Fechar histórico"><X className="h-3 w-3"/></button></header>
