...
export function OperationalDataGrid<T extends { id: string }>({ module, userId, brandId, rows, columns, loading = false, error = null,
  ...
  return <section className="flex min-h-0 flex-1 flex-col overflow-hidden border-y border-divider bg-background" aria-label={`Planilha ${module}`}>
    ...
    {!topbar ? <div className="flex shrink-0 items-center gap-1.5 overflow-x-auto border-b border-divider bg-background p-2">
      {title && <div className="mr-2 min-w-36 shrink-0"><h1 className="text-[12px] font-bold uppercase tracking-wider text-foreground">{title}</h1>{description && <p className="max-w-56 truncate text-[12px] text-text-muted">{description}</p>}</div>}
      <label className="relative min-w-44 flex-1 max-w-sm shrink-0"><Search className="pointer-events-none absolute left-2 top-2 h-3 w-3 text-text-muted"/><input value={search} onChange={event => setSearchValue(event.target.value)} placeholder={searchPlaceholder} className={`${control} w-full pl-7`}/></label>
      ...
      <div className="relative"><button className={control} onClick={() => setShowColumns(value => !value)} title="Exibir ou ocultar colunas"><Columns3 className="h-3 w-3"/></button>{showColumns && <div className="absolute right-0 z-50 mt-1 w-52 rounded border border-divider bg-surface-elevated p-2 shadow-xl">{columns.map(column => <label key={column.id} className="flex items-center gap-2 py-1 text-[12px]"><input type="checkbox" checked={!hidden.has(column.id)} onChange={() => toggleColumn(column.id)}/>{column.header}</label>)}</div>}</div>
    ...
    {error ? <div className="m-4 rounded border border-danger/45 bg-danger-soft p-4 text-sm text-danger">{error}</div> : loading ? <div className="flex min-h-44 items-center justify-center text-sm text-text-muted">Carregando planilha…</div> : !queried.length ? <div className="flex min-h-44 items-center justify-center text-sm text-text-muted">{emptyTitle}</div> : <div ref={scrollRef} onScroll={event => window.localStorage.setItem(`${storageKey}:scroll`, String(event.currentTarget.scrollTop))} className="min-h-0 flex-1 overflow-auto">
      <table className="w-full table-fixed border-collapse text-sm" style={{ minWidth: Math.max(960, visibleColumns.reduce((total, column) => total + (widths[column.id] || column.width || 140), 150)) }}>
        <thead className="sticky top-0 z-30 bg-surface-subtle text-text-muted"><tr><th className="sticky left-0 z-40 w-10 border-b border-r border-divider bg-surface-subtle p-1 text-center">#</th><th className="sticky left-10 z-40 w-9 border-b border-r border-divider bg-surface-subtle p-1"><input ref={selectAllRef} type="checkbox" checked={bulkSelection.checked} onPointerDown={event => event.stopPropagation()} onClick={event => event.stopPropagation()} onChange={event => { const checked = event.target.checked; commitBulkSelection(selectAllVisible(bulkSelected, visibleIds, checked), { kind: "visible", rowIds: visibleIds, checked }); }}/></th><th className="w-7 border-b border-divider" title={sort ? "A ordenação manual está bloqueada pela ordenação automática." : "Arrastar para reordenar"}><GripVertical className="mx-auto h-3 w-3"/></th>
          ...
          {renderActions && <th className="sticky right-0 z-40 w-40 border-b border-l border-divider bg-surface-subtle p-2 text-right">Ações</th>}
        </tr></thead>
        <tbody>{pageRows.map((row, index) => <React.Fragment key={row.id}><tr draggable={orderMode === "manual" && !sort} tabIndex={onRowActivate ? 0 : undefined} aria-current={activeRowId !== undefined && activeRowId === row.id ? "true" : undefined} data-focused-row={activeRowId === row.id ? "true" : undefined} data-bulk-selected={bulkSelected.has(row.id) ? "true" : undefined} onClick={onRowActivate ? event => { const target = event.target as HTMLElement; if (target.closest("button, input, a, select, textarea, [role=\"button\"]")) return; onRowActivate(row); } : undefined} onKeyDown={onRowActivate ? event => { if ((event.key === "Enter" || event.key === " ") && event.target === event.currentTarget) { event.preventDefault(); onRowActivate(row); } } : undefined} onDragStart={() => setDraggedId(row.id)} onDragOver={event => event.preventDefault()} onDrop={() => { if (!draggedId) return; const next = reorderIds(manualOrder, draggedId, row.id, Boolean(sort)); setManualOrder(next); onRowOrderChange?.(next); setDraggedId(null); }} className={`border-b border-divider ${onRowActivate ? "cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-focus" : "hover:bg-surface-subtle"} ${activeRowId === row.id ? activeRowClassName : ""} ${bulkSelected.has(row.id) ? bulkSelectedRowClassName : ""}`}>
          <td className="sticky left-0 z-20 border-r border-divider bg-surface-subtle p-2 text-center text-text-muted">{(currentPage - 1) * safePageSize + index + 1}</td><td className="sticky left-10 z-20 border-r border-divider bg-surface-subtle p-2 text-center"><input type="checkbox" checked={bulkSelected.has(row.id)} onPointerDown={event => event.stopPropagation()} onClick={event => event.stopPropagation()} onChange={event => { const checked = event.target.checked; const next = new Set(bulkSelected); if (checked) next.add(row.id); else next.delete(row.id); commitBulkSelection(next, { kind: "row", rowId: row.id, checked }); }}/></td><td className="p-0 text-center text-text-muted">{renderExpanded ? <button type="button" className="flex h-full w-full cursor-pointer items-center justify-center py-2" aria-label={`${(expandedRowId !== undefined ? expandedRowId === row.id : expanded.has(row.id)) ? "Fechar" : "Abrir"} detalhes`} onPointerDown={event => event.stopPropagation()} onClick={event => { event.stopPropagation(); const isOpen = expandedRowId !== undefined ? expandedRowId === row.id : expanded.has(row.id); const nextId = isOpen ? null : row.id; if (onExpandedRowChange) onExpandedRowChange(nextId); else setExpanded(current => { const next = new Set(current); if (next.has(row.id)) next.delete(row.id); else next.add(row.id); return next; }); }}>{(expandedRowId !== undefined ? expandedRowId === row.id : expanded.has(row.id)) ? <ChevronDown className="h-3 w-3"/> : <ChevronRight className="h-3 w-3"/>}</button> : orderMode === "manual" ? <GripVertical className="mx-auto my-2 h-3 w-3 cursor-grab"/> : null}</td>
          {visibleColumns.map((column, columnIndex) => { const sticky = column.pinned === "left" || columnIndex === 0; return <td key={column.id} style={{ left: sticky ? 79 : undefined }} className={`${sticky ? "sticky z-10 bg-surface-subtle" : ""} truncate border-r border-divider p-2 align-top`}>{column.render ? column.render(row) : String(column.value(row) ?? "—")}</td>; })}
          {renderActions && <td className="sticky right-0 z-20 border-l border-divider bg-surface-subtle p-1.5 text-right">{renderActions(row)}</td>}
        </tr>{(expandedRowId !== undefined ? expandedRowId === row.id : expanded.has(row.id)) && renderExpanded && <tr className="border-b border-divider"><td colSpan={visibleColumns.length + (renderActions ? 4 : 3)} className="border-l-2 border-l-module-accent bg-background p-4">{renderExpanded(row)}</td></tr>}</React.Fragment>)}</tbody>
      </table>
    </div>}
    <footer className={`flex shrink-0 items-center justify-between gap-2 overflow-x-auto border-t px-3 py-1.5 text-[12px] ${bulkSelected.size > 0 ? "border-divider bg-selected text-context-accent" : "border-divider text-text-muted"}`}><div className="flex shrink-0 items-center gap-2"><strong>{queried.length} item(ns) · {bulkSelected.size} selecionado(s)</strong>{bulkSelected.size > 0 && <>{renderBulkBar ? renderBulkBar(rows.filter(row => bulkSelected.has(row.id))) : bulkActions.map(action => <button key={action.label} disabled={action.disabled} className={control} onClick={() => void action.onClick(rows.filter(row => bulkSelected.has(row.id)))}>{action.label}</button>)}<button className={control} onClick={() => exportRows(rows.filter(row => bulkSelected.has(row.id)), "selecionados")}><Download className="mr-1 inline h-3 w-3"/>Exportar selecionados</button><button className={control} onClick={() => commitBulkSelection(new Set(), { kind: "clear" })}>Limpar seleção</button></>}</div><div className="flex shrink-0 items-center gap-1"><button className={control} disabled={currentPage <= 1} onClick={() => setPage(value => Math.max(1, value - 1))}>Anterior</button><span>{currentPage}/{pages}</span><button className={control} disabled={currentPage >= pages} onClick={() => setPage(value => Math.min(pages, value + 1))}>Próxima</button><button className={control} onClick={resetSystem}><Settings2 className="mr-1 inline h-3 w-3"/>Restaurar</button></div></footer>
  </section>;
}
