...
export const btn = "inline-flex h-7 items-center rounded border border-divider bg-surface-subtle px-2.5 text-[12px] font-bold text-foreground/75 transition-colors hover:border-module-accent/30 hover:bg-surface-elevated hover:text-foreground disabled:cursor-not-allowed disabled:opacity-40";
export const field = "h-8 w-full rounded border border-divider bg-surface-subtle px-2 text-[12px] text-foreground outline-none transition-colors hover:border-module-accent/25 focus:border-module-accent/45";
export const card = "rounded-lg border border-divider bg-surface-subtle p-4";
...
export const Field = ({ label, value, tone }: { label: string; value: unknown; tone?: "keyword" }) => <div><dt className="text-[12px] font-bold uppercase tracking-wider text-text-muted">{label}</dt><dd className={`mt-1 break-words text-[12px] ${tone === "keyword" ? "text-keyword" : "text-foreground"}`}>{value === null || value === undefined || value === "" ? "Não informado" : String(value)}</dd></div>;
...
export function ModuleHeader({ title, description, actions, tone = "default", brandName }: { title: string; description: string; actions?: React.ReactNode; tone?: "default" | "neutral"; brandName?: string }) {
  ...
  return <header className="sticky top-0 z-40 flex h-12 max-w-full items-center justify-between gap-2 overflow-hidden border-b border-divider bg-background/95 px-3 backdrop-blur"><div className="min-w-0 flex-1"><p className={`truncate text-[12px] font-bold uppercase tracking-[.2em] ${tone === "neutral" ? "text-text-muted" : "text-context-accent"}`}>{brandName || activeBrand?.nome || "Sem marca"}</p><div className="flex min-w-0 items-baseline gap-2"><h1 className="truncate text-sm font-bold text-foreground">{title}</h1><p className="hidden truncate text-[12px] text-text-muted md:block">{description}</p></div></div><div className="flex min-w-0 shrink-0 items-center gap-1">{actions}</div></header>;
}
...
export function useReadyPipeline() {
  ...
  if (shouldWaitForRadarBrandBootstrap({ selectedBrandId, profileLoading, pipelineLoading: pipeline.loading, hasSnapshot: Boolean(pipeline.snapshot), error: pipeline.error })) return { pipeline, state: <div className="flex min-h-[50vh] items-center justify-center text-xs text-text-muted"><Loader2 className="mr-2 h-4 w-4 animate-spin"/>Carregando…</div> };
...
export function LocalCollection({ title, count, description }: { title: string; count: number; description: string }) { return <section className={card}><h2 className="text-xs font-bold">{title}</h2><p className="mt-2 text-2xl font-black text-foreground">{count}</p><p className="mt-2 text-[12px] text-text-muted">{description}</p></section>; }
