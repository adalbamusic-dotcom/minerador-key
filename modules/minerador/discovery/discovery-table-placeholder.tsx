...
import { ArrowDown, ArrowUp, ArrowUpDown, Download, LoaderCircle, Play, Search, Send, X } from "lucide-react";
...
import type { DiscoveryFunnel, DiscoveryIntent } from "./discovery-types";
...
const discoveryColumnWidths = {
  drag: 32, selection: 34, keyword: 380, relation: 128, results: 112, volume: 112, trend: 112,
  cpc: 112, competition: 170, intent: 170, funnel: 130, targeting: 200, situation: 160,
};
const discoveryColumnConstraints = {
  drag: { min: 28, max: 48 }, selection: { min: 30, max: 56 }, keyword: { min: 240, max: 900, flexible: true }, relation: { min: 96, max: 220, flexible: true },
  results: { min: 80, max: 180 }, volume: { min: 96, max: 220 }, trend: { min: 96, max: 220 }, cpc: { min: 96, max: 220 },
  competition: { min: 140, max: 300 }, intent: { min: 140, max: 300, flexible: true }, funnel: { min: 110, max: 240 }, targeting: { min: 160, max: 360, flexible: true }, situation: { min: 130, max: 300 },
};
const auxiliaryCell = "border-r border-divider/70 px-2 py-1";
const keywordCell = "max-w-[380px] truncate border-r border-divider/70 px-3 py-1 font-medium text-foreground select-text";
...
export function DiscoveryTablePlaceholder({ candidates = [], intent, funnel, seed = "", brandRef, onCandidatesPatched }: { candidates?: DiscoveryCandidate[]; intent?: DiscoveryIntent; funnel?: DiscoveryFunnel; seed?: string; brandRef: string; onCandidatesPatched?: (candidateId: string, patch: DiscoveryCandidatePatch) => void }) {
  ...
  const responsiveWidths = useKeywordTableResponsiveWidths(columnResize.widths, discoveryColumnConstraints, tableRef);
  ...
  const organizedCandidates = useMemo(() => {
    const filtered = candidatesWithImportState.filter(candidate => candidateMatchesDiscoveryOrganization(candidate, seed, intent || "Não definida", funnel || "Não definido", selection.selectedIds, search, filters));
    if (orderMode === "manual") return applyManualOrder(filtered, candidateOrder.manualOrderIds, candidate => candidate.candidateId);
    return sortDiscoveryCandidates(filtered, seed, intent || "Não definida", funnel || "Não definido", primarySort, secondarySort);
  }, [candidatesWithImportState, seed, intent, funnel, selection.selectedIds, search, filters, orderMode, candidateOrder.manualOrderIds, primarySort, secondarySort]);
  ...
  const allSelectedHaveAllintitle = selectedCandidates.length > 0 && selectedCandidates.every(candidate => typeof candidate.currentMetrics?.resultsAllintitle === "number" && candidate.currentMetrics.resultsAllintitle >= 0);
  const someSelectedHaveAllintitle = selectedCandidates.some(candidate => typeof candidate.currentMetrics?.resultsAllintitle === "number" && candidate.currentMetrics.resultsAllintitle >= 0);
  ...
  const measureAllintitle = async () => {
    ...
    setInlineNotice(null);
    publishNotice({ severity: "PENDING", title: "Medição allintitle", message: `Medindo resultados para ${selectedCandidateIds.length} candidata(s)…`, source: "workflow" });
    try {
      const response = await fetch(`/api/minerador/marcas/${encodeURIComponent(brandId)}/dataforseo/allintitle`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ operationRequestId: crypto.randomUUID(), candidateIds: selectedCandidateIds }) });
      const payload = await response.json() as { success?: boolean; message?: string; projections?: Array<{ targetId?: string; candidateId?: string | null; resultsAllintitle?: number; measuredAt?: string }> };
      if (!response.ok || payload.success !== true) throw new Error(payload.message || "A medição allintitle não foi concluída.");
      for (const item of payload.projections || []) {
        const candidateId = item.candidateId || item.targetId;
        if (candidateId && typeof item.resultsAllintitle === "number") patchCandidate(candidateId, { currentMetrics: { resultsAllintitle: item.resultsAllintitle, allintitleStatus: "measured", allintitleMeasuredAt: item.measuredAt || null, allintitleProvider: "dataforseo", allintitleExecutor: "minerador_server", allintitleErrorCode: null, allintitleErrorMessage: null } });
      }
      publishNotice({ severity: "SUCCESS", title: "Medição allintitle concluída", message: payload.message || "Resultados allintitle atualizados nas candidatas selecionadas.", source: "persistence", confirmed: true });
    } catch (error) {
      publishNotice({ severity: "ERROR", title: "Medição allintitle", message: error instanceof Error ? error.message : "A medição allintitle não foi concluída.", source: "persistence", details: process.env.NODE_ENV !== "production" && error instanceof Error ? error.message : undefined });
  ...
  return <section className="mt-5 flex min-h-0 w-full flex-1 flex-col" aria-label="Resultados da descoberta">
    ...
    <KeywordTableShell ref={tableRef} scroll="x" className={selection.selectedIds.size ? "pb-14" : ""}>
      <table data-keyword-table="discovery" className="w-full min-w-[1442px] table-fixed border-collapse text-left text-sm font-sans whitespace-nowrap">
        <colgroup>{Object.keys(discoveryColumnWidths).map(columnId => <col key={columnId} data-keyword-table-column={columnId} style={{ width: responsiveWidths[columnId] }} />)}</colgroup>
        <KeywordTableHeader className="sticky top-0 z-20 border-b border-divider bg-surface-subtle"><tr className="text-text-muted"><th className="relative w-8 border-r border-divider/70 px-1 py-2" aria-label="Reordenar linhas"><KeywordTableColumnResizeHandle columnId="drag" label="reordenação" onStart={columnResize.startResize} /></th><KeywordSelectionHeader allSelected={visibleIds.length > 0 && visibleSelected === visibleIds.length} someSelected={visibleSelected > 0 && visibleSelected < visibleIds.length} onToggle={selection.toggleVisible} resizeHandle={<KeywordTableColumnResizeHandle columnId="selection" label="seleção" onStart={columnResize.startResize} />} />{columns.map(column => <SortableHeader key={column.field} column={column} sort={orderMode === "manual" ? null : primarySort} onClick={() => cycleSort(column.field)} onResize={columnResize.startResize} />)}</tr></KeywordTableHeader>
        <tbody className="divide-y divide-divider/70 bg-background">
          {organizedCandidates.map(candidate => {
            ...
            return <tr key={candidate.candidateId} data-keyword-table-row-id={candidate.candidateId} style={{ height: rowResize.getHeight(candidate.candidateId) }} onDragOver={event => handleCandidateRowDragOver(event, candidate.candidateId)} onDrop={handleCandidateRowDrop} className={`transition-colors ${candidateOrder.draggingId === candidate.candidateId ? "bg-surface-elevated opacity-70" : selection.selectedIds.has(candidate.candidateId) ? "bg-selected hover:bg-surface-elevated" : "hover:bg-surface-subtle"}`}><td className="w-8 border-r border-divider/70 px-0.5 py-1 text-center"><KeywordTableDragHandle id={candidate.candidateId} label={candidate.keyword} enabled={orderMode === "manual"} onDragStart={candidateOrder.startDragging} onDragEnd={candidateOrder.endDragging} onPointerDragStart={candidateOrder.startPointerDragging} onMouseDragStart={candidateOrder.startMouseDragging} onKeyboardMove={(id, offset) => candidateOrder.moveByOffset(id, offset)} /></td><KeywordSelectionCell id={candidate.candidateId} keyword={candidate.keyword} selected={selection.selectedIds.has(candidate.candidateId)} onPointerDown={event => selection.onSelectionPointerDown(candidate.candidateId, event)} onClick={event => selection.onSelectionClick(candidate.candidateId, event)} /><td className={keywordCell} title={candidate.keyword} aria-label={`${candidate.keyword} · Origem: ${sourceLabel}`}><div className="flex min-w-0 items-center gap-2"><span className="min-w-0 truncate select-text cursor-text">{candidate.keyword}</span><span className={sourceBadge} title={`Origem: ${sourceLabel}`}>{sourceLabel}</span></div></td><td className={auxiliaryCell}>{relationText(classifyDiscoveryRelation(seed, candidate.keyword))}</td><td className={auxiliaryCell}>{allintitle === null || allintitle === undefined ? "—" : new Intl.NumberFormat("pt-BR").format(allintitle)}</td><td className={auxiliaryCell}>{candidate.averageMonthlySearches === null ? "Sem média oficial" : new Intl.NumberFormat("pt-BR").format(candidate.averageMonthlySearches)}</td><td className={auxiliaryCell}>{trendText(discoveryTrend(candidate))}</td><td className={auxiliaryCell}>{formatDiscoveryMoney(candidate.averageCpcMicros, candidate.currencyCode)}</td><td className={auxiliaryCell}>{competitionText(candidate.competition)}{candidate.competitionIndex === null ? "" : ` · ${candidate.competitionIndex}`}</td><td className={auxiliaryCell}>{intent || "Não definida"}</td><td className={auxiliaryCell}>{funnel || "Não definido"}</td><td className={`${auxiliaryCell} max-w-[190px] truncate`} title={targeting.details.join("; ")}>{targeting.label}</td><td className="relative px-2 py-1">{situationText(discoverySituation(candidate))}<KeywordTableRowResizeHandle rowId={candidate.candidateId} enabled onStart={rowResize.startResize} /></td></tr>;
          })}
          {!organizedCandidates.length && <tr><td colSpan={columns.length + 2} className="p-0"><KeywordTableEmptyState>{candidates.length ? <p className="text-sm text-text-muted">Nenhuma candidata corresponde à organização local.</p> : <DiscoveryEmptyState />}</KeywordTableEmptyState></td></tr>}
        </tbody>
      </table>
    </KeywordTableShell>
    {selection.selectedIds.size > 0 && <KeywordTableBulkBarShell><strong className="shrink-0 rounded border border-module-accent/50 bg-selected px-1.5 py-1 text-xs font-bold text-foreground sm:px-2.5 sm:text-sm">{selection.selectedIds.size} selecionada(s)</strong><span className="hidden shrink-0 text-xs text-text-muted xl:inline">{selectedNewCount} nova(s) · {selectedExistingCount} já existente(s)</span><button type="button" onClick={() => void importSelected()} disabled={importing} title="Enviar somente as candidatas selecionadas ao Processador" className="inline-flex min-h-9 shrink-0 items-center gap-1 rounded border border-action-accent bg-action-accent px-2 py-1 text-xs font-semibold text-foreground transition-colors hover:bg-action-accent/85 disabled:cursor-wait disabled:opacity-60 sm:px-2.5 sm:text-sm">{importing ? <LoaderCircle className="h-3.5 w-3.5 animate-spin" aria-hidden="true" /> : <Send className="h-3.5 w-3.5" aria-hidden="true" />}<span className="hidden xl:inline">{importing ? "Enviando..." : "Enviar selecionadas ao Processador"}</span></button><button type="button" onClick={() => void measureAllintitle()} disabled={importing || volumeMeasuring || allintitleMeasuring} title="Executar a medição allintitle paga somente nas candidatas selecionadas" className="inline-flex min-h-9 shrink-0 items-center gap-1 rounded border border-context-accent/50 px-2 py-1 text-xs font-semibold text-context-accent hover:bg-context-accent/10 disabled:cursor-wait disabled:opacity-60 sm:px-2.5 sm:text-sm">{allintitleMeasuring ? <LoaderCircle className="h-3.5 w-3.5 animate-spin" aria-hidden="true" /> : <Play className="h-3.5 w-3.5" aria-hidden="true" />}<span className="hidden xl:inline">{allintitleMeasuring ? "Medindo resultados..." : allSelectedHaveAllintitle ? "Atualizar resultados" : someSelectedHaveAllintitle ? "Medir/Atualizar resultados" : "Medir resultados"}</span></button><button type="button" onClick={() => void measureVolume()} disabled={importing || volumeMeasuring || allintitleMeasuring} title="Atualizar as métricas das candidatas selecionadas" className="inline-flex min-h-9 shrink-0 items-center gap-1 rounded border border-success/50 px-2 py-1 text-xs font-semibold text-success hover:bg-success-soft disabled:cursor-wait disabled:opacity-60 sm:px-2.5 sm:text-sm">{volumeMeasuring ? <LoaderCircle className="h-3.5 w-3.5 animate-spin" aria-hidden="true" /> : <Play className="h-3.5 w-3.5" aria-hidden="true" />}<span className="hidden xl:inline">{volumeMeasuring ? "Atualizando métricas..." : "Atualizar métricas"}</span></button><button type="button" disabled title="A exportação será habilitada em fase posterior." className="hidden min-h-9 shrink-0 items-center gap-1 rounded border border-divider px-2.5 py-1 text-sm font-semibold text-text-muted xl:inline-flex"><Download className="h-3.5 w-3.5" aria-hidden="true" />Exportar</button><button type="button" onClick={() => selection.setSelectedIds(new Set())} disabled={importing || volumeMeasuring || allintitleMeasuring} className="ml-auto inline-flex min-h-9 shrink-0 items-center gap-1 rounded border border-divider px-2 py-1 text-xs font-semibold text-foreground/80 hover:border-module-accent/30 hover:bg-surface-elevated hover:text-foreground sm:px-2.5 sm:text-sm"><X className="h-3.5 w-3.5" aria-hidden="true" /><span className="hidden xl:inline">Limpar seleção</span></button></KeywordTableBulkBarShell>}
  </section>;
}
...
function SortableHeader({ column, sort, onClick, onResize }: { column: { label: string; field: DiscoverySortField }; sort: DiscoverySort | null; onClick: () => void; onResize: (columnId: string, event: KeywordTableResizeStartEvent) => void }) {
  ...
  return <th aria-sort={state === "none" ? "none" : state === "asc" ? "ascending" : "descending"} className="relative border-r border-divider/70 px-3 py-2 text-sm font-medium text-text-muted last:border-r-0"><button type="button" title={`Ordenar por ${column.label}`} onClick={onClick} className="inline-flex items-center gap-1 hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-module-accent/40">{column.label}{state === "asc" ? <ArrowUp className="h-3.5 w-3.5" aria-hidden="true" /> : state === "desc" ? <ArrowDown className="h-3.5 w-3.5" aria-hidden="true" /> : <ArrowUpDown className="h-3.5 w-3.5 text-text-muted" aria-hidden="true" />}</button><KeywordTableColumnResizeHandle columnId={column.field} label={column.label} onStart={onResize} /></th>;
}

function OrganizationPanel({ filters, setFilters, orderMode, setOrderMode, primarySort, secondarySort, setPrimarySort, setSecondarySort }: { filters: DiscoveryOrganizationFilters; setFilters: (value: DiscoveryOrganizationFilters) => void; orderMode: KeywordTableOrderMode; setOrderMode: (value: KeywordTableOrderMode) => void; primarySort: DiscoverySort | null; secondarySort: DiscoverySort | null; setPrimarySort: (value: DiscoverySort | null) => void; setSecondarySort: (value: DiscoverySort | null) => void }) {
