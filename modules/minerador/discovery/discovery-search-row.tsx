...
const discoveryModeLabels: Record<DiscoverySearchKind, string> = { keyword: "Palavra-chave", customer_discovery: "Como clientes me encontram", subject: "Por Assunto" };
const discoveryModeHelpers: Record<DiscoverySearchKind, string> = { keyword: "Expanda um termo conhecido para encontrar palavras-chave relacionadas.", customer_discovery: "Descubra como potenciais clientes procuram seu produto, serviço ou nicho.", subject: "Encontre as keywords que sustentam um Assunto escrito por você ou já declarado na marca." };
...
export function DiscoverySearchRow({ searchKind, onSearchKindChange, subjectFields, discoveryFocus, setDiscoveryFocus, seed, setSeed, relation, setRelation, intent, setIntent, funnel, setFunnel, language, setLanguage, states: selectedStates, setStates, onSubmit }: {
  /** Opção do radiogroup. "subject" é só de tela: a busca por Assunto tem rota própria. */
  searchKind: DiscoverySearchKind;
  onSearchKindChange: (kind: DiscoverySearchKind) => void;
  /** Campos da Pesquisa por Assunto, no lugar da semente, quando `searchKind` é "subject". */
  subjectFields?: ReactNode;
  ...
  const customerDiscovery = searchKind === "customer_discovery";
  const subjectSearch = searchKind === "subject";
  ...
  return <section className="min-w-0 w-full max-w-full border-b border-divider px-3 py-4 sm:px-4 xl:px-6" aria-label="Busca de keywords">
    <fieldset className="mb-4 min-w-0 max-w-full">
      ...
      <p id="discovery-mode-help" className="mt-1 max-w-[60ch] text-sm font-normal leading-5 text-text-muted">{discoveryModeHelpers[searchKind]}</p>
      <div role="radiogroup" aria-label="Tipo de descoberta" aria-describedby="discovery-mode-help" className="mt-3 inline-flex max-w-full overflow-x-auto rounded border border-divider bg-surface-subtle">
        {DISCOVERY_SEARCH_KINDS.map(option => {
          const selected = searchKind === option;
          return <label key={option} className={`group relative inline-flex min-h-8 shrink-0 items-center border-r border-divider last:border-r-0 ${selected ? "bg-module-accent/10 text-foreground" : "text-text-muted hover:bg-surface-elevated hover:text-foreground"}`}>
            <input type="radio" name="discovery-mode" value={option} checked={selected} onChange={() => onSearchKindChange(option)} className="peer sr-only" />
    ...
    {subjectSearch && subjectFields}
    <div className={subjectSearch ? "mt-4 grid min-w-0 gap-3 md:grid-cols-2 xl:grid-cols-4 xl:items-end" : "grid min-w-0 gap-3 md:grid-cols-2 xl:grid-cols-[minmax(0,1.8fr)_minmax(10rem,max-content)_minmax(9rem,.8fr)_minmax(8rem,.65fr)_minmax(7rem,.6fr)_minmax(6rem,.55fr)_minmax(9rem,.85fr)_minmax(7rem,.55fr)] xl:items-end"}>
      {!subjectSearch && <>
