...
export function DiscoveryFilterRow({ discoveryMode, volume, setVolume, cpc, setCpc, resultPreset, setResultPreset, resultMin, setResultMin, resultMax, setResultMax, keywordDifficultyPreset, setKeywordDifficultyPreset, keywordDifficultyMin, setKeywordDifficultyMin, keywordDifficultyMax, setKeywordDifficultyMax, hasSeoData, serpMeasurementEnabled = false, includeTerms, setIncludeTerms, excludeTerms, setExcludeTerms, includeAdultKeywords, setIncludeAdultKeywords, activeFilterPopover, setActiveFilterPopover, onDiscover, onClear, loading, canSubmit }: { discoveryMode: DiscoveryMode; volume: DiscoveryVolumeRange; setVolume: Dispatch<SetStateAction<DiscoveryVolumeRange>>; cpc: DiscoveryCpcFilter; setCpc: Dispatch<SetStateAction<DiscoveryCpcFilter>>; resultPreset: DiscoveryResultPreset; setResultPreset: Dispatch<SetStateAction<DiscoveryResultPreset>>; resultMin: string; setResultMin: Dispatch<SetStateAction<string>>; resultMax: string; setResultMax: Dispatch<SetStateAction<string>>; keywordDifficultyPreset: DiscoveryKeywordDifficultyPreset; setKeywordDifficultyPreset: Dispatch<SetStateAction<DiscoveryKeywordDifficultyPreset>>; keywordDifficultyMin: string; setKeywordDifficultyMin: Dispatch<SetStateAction<string>>; keywordDifficultyMax: string; setKeywordDifficultyMax: Dispatch<SetStateAction<string>>; hasSeoData: boolean; serpMeasurementEnabled?: boolean; includeTerms: string; setIncludeTerms: Dispatch<SetStateAction<string>>; excludeTerms: string; setExcludeTerms: Dispatch<SetStateAction<string>>; includeAdultKeywords: boolean; setIncludeAdultKeywords: Dispatch<SetStateAction<boolean>>; activeFilterPopover: DiscoveryFilterPopover; setActiveFilterPopover: Dispatch<SetStateAction<DiscoveryFilterPopover>>; onDiscover: () => void | Promise<void>; onClear: () => void; loading: boolean; canSubmit: boolean; }) {
  ...
  return <section className="flex flex-wrap gap-2 border-b border-divider px-3 py-4 sm:px-4 xl:px-6" aria-label="Filtros da pesquisa e do enriquecimento SEO">
      <NumericRangeFilter label="Resultado" name="discovery-result" preset={resultPreset} setPreset={setResultPreset} presets={DISCOVERY_RESULT_PRESETS} min={resultMin} max={resultMax} setMin={setResultMin} setMax={setResultMax} hasData={hasSeoData} serpMeasurementEnabled={serpMeasurementEnabled} triggerRef={resultTriggerRef} open={activeFilterPopover === "result"} onToggle={() => togglePopover("result")} onClose={closePopover} />
...
function numericRangeFilterTitle(label: string, missingSelected: boolean, serpMeasurementEnabled: boolean, hasData: boolean) {
  if (missingSelected) return serpMeasurementEnabled ? "Sem medição neste filtro. Medir resultados já está liberado pelo outro filtro SEO" : "Sem medição: Medir resultados fica bloqueado. Escolha outra opção para liberar a DataForSEO";
  return hasData ? `Filtrar ${label} localmente sobre dados SEO já medidos` : "Medir resultados liberado para as candidatas selecionadas";
}

function NumericRangeFilter<P extends string>({ label, name, preset, setPreset, presets, min, max, setMin, setMax, maxBound, helperText, hasData, serpMeasurementEnabled, triggerRef, open, onToggle, onClose }: { label: string; name: string; preset: P; setPreset: Dispatch<SetStateAction<P>>; presets: readonly { key: P; label: string }[]; min: string; max: string; setMin: Dispatch<SetStateAction<string>>; setMax: Dispatch<SetStateAction<string>>; maxBound?: number; helperText?: string; hasData: boolean; serpMeasurementEnabled: boolean; triggerRef: RefObject<HTMLButtonElement | null>; open: boolean; onToggle: () => void; onClose: () => void }) {
  const selectedLabel = presets.find(option => option.key === preset)?.label || "Todos";
  const presentLabel = presets.find(option => option.key === "present")?.label || "Com medição";
  const customSelected = preset === "custom";
  return <>
    <button ref={triggerRef} type="button" onClick={onToggle} aria-expanded={open} className={filterButton} title={numericRangeFilterTitle(label, preset === "missing", serpMeasurementEnabled, hasData)}>{label} · {selectedLabel}<ChevronDown className="h-4 w-4 text-text-muted" aria-hidden="true" /></button>
    <AnchoredPopover open={open} onClose={onClose} triggerRef={triggerRef} ariaLabel={`Filtro de ${label}`}>
      <div className="grid gap-3">
        <div>
          <p className="text-sm font-medium text-foreground">{label}</p>
          <p className="mt-1 text-sm leading-5 text-text-muted">{hasData ? "Filtro local sobre dados já medidos." : "Ainda não há dados SEO medidos nesta lista."}</p>
          <p className="mt-1 text-sm leading-5 text-text-muted">Sem medição é o padrão e mantém Medir resultados bloqueado. Qualquer outra opção, aqui ou no outro filtro SEO, libera a medição DataForSEO. Escolha Todos para liberar e continuar vendo as candidatas sem medição; faixas e {presentLabel} mostram só as já medidas.</p>
