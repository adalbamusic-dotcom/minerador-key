...
export function BrandDnaPanel({ brandId, legacyGuidelines, niche }: { brandId: string; legacyGuidelines: string; niche: string }) {
  ...
  useNoticeBridge({ notice: message, module: "marca", area: "Identidade da marca", title: "Marca · Identidade", fallbackSeverity: "INFO" });
  ...
  const save = async () => {
    ...
    try {
      ...
      const body = await response.json().catch(() => ({}));
      if (!response.ok) { if (body.code === "persistence_unavailable" && actorUserId) { window.localStorage.setItem(fallbackKey(actorUserId, brandId), JSON.stringify(draft)); setMode("local_fallback"); setMessage("Rascunho mantido localmente; nenhuma versão remota foi criada."); return; } throw new Error(body.error || "Não foi possível salvar a identidade da marca."); }
    ...
    } catch (error) { setMessage(error instanceof Error ? error.message : "Não foi possível salvar a identidade da marca."); }
  ...
  const approve = async () => {
    ...
    try {
      ...
      const body = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(body.error || "Não foi possível aprovar a identidade da marca.");
      setStatus(body.status); setMode("server"); setMessage("Identidade da marca aprovada por ação humana.");
    } catch (error) { setMessage(error instanceof Error ? error.message : "Não foi possível aprovar a identidade da marca."); }
    finally { setBusy(false); }
  };
