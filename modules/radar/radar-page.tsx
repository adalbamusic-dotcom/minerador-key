...
export function RadarPage({ brandRef }: { brandRef: string }) {
  ...
  const calcularDadosDaLinha = (row: RadarItem) => {
  ...
  /**
   * ===== RADAR_SELECTION_LIGHT_1 · UMA LEITURA POR LINHA POR RENDER =====
   *
   * `calcularDadosDaLinha` é o modelo inteiro da linha: SERP, seleção, revisão,
   * extrações, relatório consolidado. Ele era chamado em ~20 pontos — nove
   * colunas da planilha, o texto de busca do grid (todas as colunas de todas
   * as linhas), o workbench, os cards — e cada render refazia tudo. Selecionar
   * uma linha muda estado, o que renderiza de novo, o que recalculava N × 20
   * modelos para mostrar uma seleção.
   *
   * O cache vive UM render: nasce aqui e morre com ele. Sem lista de
  ...
  const rowWorkbenchData = (row: RadarItem) => {
    const pronto = cacheDaLinha.get(row);
    if (pronto) return pronto;
    const calculado = calcularDadosDaLinha(row);
    cacheDaLinha.set(row, calculado);
    return calculado;
  };
   ...
   */
  const calcularProjecaoDePesquisa = (row: RadarItem | null): RadarResearchProfileProjection =>
    radarResearchProfileStateOfAnalysis({
      payload: row ? analiseCorrenteDe(row)?.payload || null : null,
      profile: radarProfileOfTarget(modoEfetivoDe(row)),
    });
  /* Mesma disciplina do modelo da linha: uma projeção por linha por render. */
  const cacheDaProjecao = new Map<string, RadarResearchProfileProjection>();
  const projecaoDePesquisa = (row: RadarItem | null): RadarResearchProfileProjection => {
    const chave = row ? row.id : "";
    const pronta = cacheDaProjecao.get(chave);
   ...
   */
  const calcularBlueprintCanonico = (row: RadarItem | null, perfil?: RadarResearchProfile): RadarCompetitiveBlueprintView => {
  ...
  /* O blueprint é a montagem mais cara da página; três chamadas por render viravam três montagens. */
  const cacheDoBlueprint = new Map<string, RadarCompetitiveBlueprintView>();
  const blueprintCanonico = (row: RadarItem | null, perfil?: RadarResearchProfile): RadarCompetitiveBlueprintView => {
    const chave = `${row ? row.id : ""}:${perfil || ""}`;
    const pronto = cacheDoBlueprint.get(chave);
    if (pronto) return pronto;
    const calculado = calcularBlueprintCanonico(row, perfil);
    cacheDoBlueprint.set(chave, calculado);
    return calculado;
  };
...
export function LegacyRadarDetail({ row, article, pipeline }: { row: RadarItem; article?: VersionEnvelope<ArticleDNA>; pipeline: ReturnType<typeof useEditorialPipeline> }) {
  ...
  return <div className="grid gap-3 lg:grid-cols-3"><section className={card}><h3 className="text-xs font-bold">Definição do artigo e referências</h3>{article ? <dl className="mt-2 space-y-2"><Field label="Intenção" value={radarDeclaredArticleIntent(article.payload)}/><Field label="Cobertura" value={article.payload.coverage.join(" · ")}/><Field label="Keywords" value={article.payload.keywordReferences.length}/><Field label="Fronteira" value={article.payload.antiCannibalizationBoundary}/></dl> : <p className="mt-2 text-xs text-danger">Versão ausente.</p>}</section><section className={card}><h3 className="text-xs font-bold">SERP e evidências</h3>{serp ? <><DataOriginBadge origin={serp.origin}/><dl className="mt-2 space-y-2"><Field label="Intenção dominante" value={serp.snapshot?.dominantIntent}/><Field label="Perguntas" value={serp.snapshot?.questions.join(" · ")}/><Field label="Lacunas" value={serp.snapshot?.gaps.join(" · ")}/></dl></> : <ProviderNotConfiguredState/>}</section><section className={card}><h3 className="text-xs font-bold">Originalidade</h3><p className="mt-2 text-xs text-text-muted">Conflito interno: funcional.</p><p className="mt-1 text-xs text-text-muted">Similaridade externa: aguardando provedor.</p><p className="mt-1 text-xs text-text-muted">Originalidade estratégica: decisão humana.</p></section></div>;
}
...
function RadarDetail({ row, article, pipeline }: { row: RadarItem; article?: VersionEnvelope<ArticleDNA>; pipeline: ReturnType<typeof useEditorialPipeline> }) {
  ...
  return <div className="grid gap-3 lg:grid-cols-3"><section className={card}><h3 className="text-xs font-bold">Definição do artigo e referências</h3>{article ? <dl className="mt-2 space-y-2"><Field label="Intenção" value={radarDeclaredArticleIntent(article.payload)}/><Field label="Cobertura" value={article.payload.coverage.join(" · ")}/><Field label="Keywords" value={article.payload.keywordReferences.length}/><Field label="Fronteira" value={article.payload.antiCannibalizationBoundary}/></dl> : <p className="mt-2 text-xs text-danger">Versão ausente.</p>}</section><section className={`${card} lg:col-span-2`}><div className="flex items-center justify-between"><h3 className="text-xs font-bold">SERP e revisão humana</h3>{serp && <DataOriginBadge origin={serp.origin}/>}</div>{!serp && <ProviderNotConfiguredState/>}{serp && !serp.research && <p className="mt-2 text-xs text-warning">Dados simulados: não podem ser aprovados como evidência real.</p>}{serp?.research && <><div className="mt-2 grid gap-2 sm:grid-cols-4"><Field label="Consulta" value={serp.research.query}/><Field label="Captura" value={new Date(serp.research.collectedAt).toLocaleString("pt-BR")}/><Field label="Análise da SERP" value={`v${serp.research.version}`}/><Field label="Persistência" value={serp.persistenceMode === "remote" ? "remota" : "local — não confirmada"}/></div><div className="mt-3 grid gap-3 md:grid-cols-2"><section><h4 className="text-xs font-bold uppercase text-text-muted">Resultados orgânicos ({serp.research.organicResults.length})</h4><div className="mt-1 max-h-64 overflow-auto rounded border border-divider">{serp.research.organicResults.length ? serp.research.organicResults.map(result => <div key={`${serp.id}:${result.position}`} className="border-b border-divider p-2 text-xs last:border-0"><div className="flex justify-between gap-2"><strong>{result.position}. {result.title}</strong><span className="text-text-muted">{result.manualType || result.inferredType} · {result.confidence}</span></div><a className="break-all text-context-accent" href={result.url} target="_blank" rel="noreferrer">{result.domain}</a><p className="mt-1 text-text-muted">{result.snippet || "Sem snippet retornado."}</p></div>) : <p className="p-2 text-xs text-text-muted">Nenhum resultado orgânico retornado.</p>}</div></section><section><h4 className="text-xs font-bold uppercase text-text-muted">People Also Ask ({serp.research.peopleAlsoAsk.length})</h4><div className="mt-1 max-h-64 overflow-auto rounded border border-divider">{serp.research.peopleAlsoAsk.length ? serp.research.peopleAlsoAsk.map(item => <div key={`${serp.id}:paa:${item.position}`} className="border-b border-divider p-2 text-xs last:border-0"><strong>{item.question}</strong><p className="mt-1 text-text-muted">{item.answer || "Sem resposta resumida retornada."}</p></div>) : <p className="p-2 text-xs text-text-muted">Nenhuma pergunta retornada.</p>}</div><h4 className="mt-3 text-xs font-bold uppercase text-text-muted">Pesquisas relacionadas ({serp.research.relatedSearches.length})</h4><div className="mt-1 flex flex-wrap gap-1">{serp.research.relatedSearches.length ? serp.research.relatedSearches.map(item => <span key={`${serp.id}:related:${item.term}`} className="rounded border border-divider px-1.5 py-1 text-xs text-text-muted">{item.term}</span>) : <span className="text-xs text-text-muted">Nenhuma retornada.</span>}</div></section></div><section className="mt-3 rounded border border-context-accent/25 bg-context-accent/10 p-2"><h4 className="text-xs font-bold uppercase text-context-accent">Diagnóstico determinístico</h4><div className="mt-2 grid gap-2 sm:grid-cols-3"><Field label="Intenção aparente" value={serp.research.diagnostic.dominantIntent}/><Field label="Confiança" value={serp.research.diagnostic.confidence}/><Field label="Veredito" value={serp.research.diagnostic.verdict}/><Field label="Formatos" value={serp.research.diagnostic.dominantFormats.join(" · ")}/><Field label="Domínios recorrentes" value={serp.research.diagnostic.frequentDomains.join(" · ")}/><Field label="Conflitos" value={serp.research.diagnostic.possibleConflicts.join(" · ")}/></div><p className="mt-2 text-xs text-text-muted">{serp.research.diagnostic.limitations.join(" ")}</p></section><div className="mt-3 flex flex-wrap items-end gap-2"><label className="min-w-[260px] flex-1 text-xs text-text-muted">Nota da revisão<textarea value={notes} onChange={event => setNotes(event.target.value)} rows={2} className="mt-1 w-full rounded border border-divider bg-surface-subtle p-2 text-xs text-foreground/85" placeholder="Registre a decisão humana…"/></label><button className={`${btn} border-success/35 text-success`} disabled={saving} onClick={() => void decide("approved")}><CheckCircle2 className="mr-1 h-3 w-3"/>{review?.status === "approved" ? "Aprovada" : "Aprovar pesquisa"}</button><button className={`${btn} border-danger/45 text-danger`} disabled={saving} onClick={() => void decide("rejected")}><XCircle className="mr-1 h-3 w-3"/>Rejeitar pesquisa</button></div>{review && <p className="mt-2 text-xs text-text-muted">Última decisão: {review.status} por {review.reviewedBy} em {new Date(review.reviewedAt).toLocaleString("pt-BR")}. {review.notes}</p>}{message && <p className="mt-2 text-xs text-warning">{message}</p>}</>}</section><section className={card}><h3 className="text-xs font-bold">Histórico de análises</h3>{snapshots.length ? <div className="mt-2 space-y-1">{snapshots.map(snapshot => <div key={snapshot.id} className="flex justify-between gap-2 border-b border-divider py-1 text-xs"><span>{snapshot.research ? `v${snapshot.research.version} · ${snapshot.origin}` : "mock"}</span><span className="text-text-muted">{snapshot.research?.contentHash.slice(0, 10) || "local"}</span></div>)}</div> : <p className="mt-2 text-xs text-text-muted">Nenhuma pesquisa registrada.</p>}</section></div>;
}
