...
export function RadarR6ReportPanel({ report, onGenerate, onReview, onApprove }: RadarR6ReportPanelProps) {
  const status = report ? radarR6ReportStateLabel(report.state) : "Aguardando geração";
  ...
  const canApprove = radarR6CanApproveReport(report);
  return <details className="mt-3 rounded-md border border-divider bg-surface" data-testid="radar-r6-report" aria-label="Relatório consolidado do Radar">
    ...
    <div className="border-t border-divider px-3 py-3">
      {!report || report.state === "NOT_STARTED" ? <div><p className="text-sm text-text-muted">A prévia local aparece depois da revisão da SERP e da ação explícita “Gerar relatório”.</p><button type="button" className={`${button} mt-3`} onClick={onGenerate} disabled={!canGenerate}>Gerar relatório local</button></div> : <>
        <div className="flex flex-wrap items-start justify-between gap-3">
          <p className="max-w-4xl text-sm leading-6 text-foreground">{report.summary}</p>
          <span className="shrink-0 rounded-full border border-divider px-3 py-1 text-sm text-text-muted">{report.final ? "Final local" : "Prévia revisável"}</span>
        </div>
