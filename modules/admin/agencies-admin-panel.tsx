...
type Invitation = { id: string; destination_email: string; proposed_agency_name: string; plan_code: string; source: string; status: string; expires_at: string; access_expires_at?: string | null; is_operational?: boolean; isOperational?: boolean };
...
function invitationValidity(invitation: Invitation) {
  ...
  const validity = [Number.isNaN(expiry.getTime()) ? "Link até: não informado" : `Link até: ${new Intl.DateTimeFormat("pt-BR", { timeZone: "America/Sao_Paulo", day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit", hourCycle: "h23" }).format(expiry)} (Brasília)`];
...
export default function AgenciesAdminPanel() {
  ...
  const deleteInvitation = async () => {
    ...
    try {
      const response = await fetch("/api/admin/agency-invitations", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ invitationId: deleteTarget.id }) });
      const body = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(body.error || "Não foi possível excluir o convite.");
      setDeleteTarget(null);
      setNotice("Convite e registros internos relacionados excluídos. Um e-mail já entregue não pode ser retirado da caixa do destinatário.");
      await load();
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Não foi possível excluir o convite."); }
  ...
  return <><GlobalTopbarPageControls actions={pageAction}/><main className="min-w-0 p-4 sm:p-6">
    ...
    {view === "invitations" ? <section className={`${card} mt-5`}><h2 className="text-lg font-semibold">Convites que exigem ação</h2>{operationalInvitations.length ? <div className="mt-4 divide-y divide-foreground/10">{operationalInvitations.map((item) => <div key={item.id} className="flex flex-wrap items-center justify-between gap-3 py-3 text-sm"><div><p className="font-medium">{item.proposed_agency_name}</p><p className="text-foreground/65">{item.destination_email} · Plano {item.plan_code} · {item.source} · Status: {item.status} · {invitationValidity(item)}</p></div><div className="flex flex-wrap gap-2"><button type="button" disabled={saving} className={actionButton} onClick={() => void invitationAction(item.id, "rotate")}>Reenviar convite</button>{item.source === "ADMIN_INVITE" ? <button type="button" disabled={saving} className={actionButton} onClick={() => setDeleteTarget(item)}>Excluir convite</button> : <button type="button" disabled={saving} className={actionButton} onClick={() => void invitationAction(item.id, "revoke")}>Revogar</button>}</div></div>)}</div> : <p className="mt-3 text-sm text-foreground/65">Nenhum convite pendente.</p>}</section> : null}
    ...
    {view === "history" ? <section className={`${card} mt-5`}><h2 className="text-lg font-semibold">Histórico</h2>{historicalApplications.length || historicalInvitations.length ? <div className="mt-4 divide-y divide-foreground/10">{historicalApplications.map((item) => <div key={`application-${item.id}`} className="py-3 text-sm"><p className="font-medium">{item.proposed_agency_name}</p><p className="text-foreground/65">Solicitação pública · {item.status} · {planLabel(item.plan_code)}</p></div>)}{historicalInvitations.map((item) => <div key={`invitation-${item.id}`} className="flex flex-wrap items-center justify-between gap-3 py-3 text-sm"><div><p className="font-medium">{item.proposed_agency_name}</p><p className="text-foreground/65">{item.destination_email} · {historicalInvitationLabel(item)} · {invitationValidity(item)}</p></div>{item.source === "ADMIN_INVITE" && item.status !== "ACCEPTED" ? <button type="button" disabled={saving} className={actionButton} onClick={() => setDeleteTarget(item)}>Excluir convite</button> : null}</div>)}</div> : <p className="mt-3 text-sm text-foreground/65">Nenhum registro histórico.</p>}</section> : null}
    {deleteTarget ? <div role="dialog" aria-modal="true" aria-labelledby="delete-invitation-title" className="fixed inset-0 z-50 grid place-items-center bg-background/85 p-4"><section className={`${card} w-full max-w-lg p-5`}><h2 id="delete-invitation-title" className="text-lg font-semibold">Excluir convite definitivamente?</h2><p className="mt-3 text-sm text-foreground/75">{deleteTarget.proposed_agency_name} · {deleteTarget.destination_email}</p><p className="mt-3 text-sm text-foreground/75">O convite, seus tokens, mensagens e eventos internos serão removidos do banco. Esta ação não pode ser desfeita. Um e-mail já enviado permanece na caixa do destinatário, mas o link deixará de funcionar.</p><div className="mt-5 flex flex-wrap gap-2"><button type="button" disabled={saving} className={actionButton} onClick={() => setDeleteTarget(null)}>Cancelar</button><button type="button" disabled={saving} className={actionButton} onClick={() => void deleteInvitation()}>{saving ? "Excluindo..." : "Excluir definitivamente"}</button></div></section></div> : null}
